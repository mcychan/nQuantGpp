...
namespace Peano
{
	...
	float normalDistribution(float x, double peak) {
		...
		auto pdf = (1 / (stdDev * sqrt(2 * M_PI))) * exp(exponent);
		auto maxPdf = 1 / (stdDev * sqrt(2 * M_PI)); // Peak at x = mean
		auto scaledPdf = (pdf / maxPdf) * peak;
		return (float) std::max(0.0, std::min(peak, scaledPdf));
	}

	ushort ditherPixel(int x, int y, Vec4b c2, float beta)
	{
		const int bidx = x + y * m_width;
		...
		auto gamma = (nMaxColors <= 32 && m_weight < .01 && m_weight > .007) ? 1 - beta : beta;
		if (nMaxColors > 4 && CIELABConvertor::Y_Diff(pixel, c2) > (gamma * acceptedDiff)) {
			if (margin > 6 || gamma > beta) {
				...
				if (nMaxColors > 32 && m_saliencies[bidx] < .9)
					kappa = beta * normalDistribution(m_saliencies[bidx], 2.0);
				else {
					...
					else if (nMaxColors >= 32 || CIELABConvertor::Y_Diff(c1, c2) > (gamma * M_PI * acceptedDiff)) {
			...
			else if (nMaxColors <= 32 && m_weight >= .004)
		...
		if (nMaxColors > 32 && m_saliencies[bidx] > .95) {
			auto kappa = beta * max(.05f, .75f - nMaxColors / 128.0f) * m_saliencies[bidx];
			c2 = BlueNoise::diffuse(pixel, qPixel, kappa, strength, x, y);
		}
	...
	void GilbertCurve::dither(const Mat4b pixels4b, const Mat palette, DitherFn ditherFn, GetColorIndexFn getColorIndexFn, Mat qPixels, float* saliencies, double weight, bool dither)
	{
		...
		if (nMaxColors > 4) {
			...
			if (nMaxColors >= 64 && (weight > .012 && weight < .0125) || (weight > .025 && weight < .03))
				...
				beta = .55f;
			else if (nMaxColors > 16 && nMaxColors <= 32 && weight < .025 && weight >= .005)
