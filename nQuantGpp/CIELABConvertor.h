...
public:
	struct Lab {
		uchar alpha = UCHAR_MAX;
		double A = 0.0;
		double B = 0.0;
		double L = 0.0;
	};
	
	static void LAB2RGB(Vec4b& pixel, const Lab& lab);
	static void RGB2LAB(const Vec4b& c1, Lab& lab);
	...
	static double Y_Diff(const Vec4b& c1, const Vec4b& c2);
	static double U_Diff(const Vec4b& c1, const Vec4b& c2);
};
