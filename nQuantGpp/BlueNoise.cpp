...
{	    
	...
	Vec4b diffuse(const Vec4b& pixel, const Vec4b& qPixel, const float weight, const float strength, const int x, const int y)
	{
		...
		adj *= weight;
		b_pix = static_cast<uchar>(min(UCHAR_MAX, (int) max(b_pix + (adj * (b_pix - qPixel[0])), 0.0f)));
		g_pix = static_cast<uchar>(min(UCHAR_MAX, (int) max(g_pix + (adj * (g_pix - qPixel[1])), 0.0f)));
		r_pix = static_cast<uchar>(min(UCHAR_MAX, (int) max(r_pix + (adj * (r_pix - qPixel[2])), 0.0f)));
		a_pix = static_cast<uchar>(min(UCHAR_MAX, (int) max(a_pix + (adj * (a_pix - qPixel[3])), 0.0f)));

		return Vec4b(b_pix, g_pix, r_pix, a_pix);
	}
	
	void dither(const Mat4b pixels4b, const Mat palette, DitherFn ditherFn, GetColorIndexFn getColorIndexFn, Mat1b qPixels, const float weight)
	{
		...
		for (uint y = 0; y < height; ++y) {
			for (uint x = 0; x < width; ++x) {
				...
				Vec4b c1;
				GrabPixel(c1, palette, qPixelIndex, 0);

				c1 = diffuse(pixel, c1, weight, strength, y, x);
