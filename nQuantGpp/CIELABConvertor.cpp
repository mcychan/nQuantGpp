...
void CIELABConvertor::RGB2LAB(const Vec4b& c1, Lab& lab)
{
	auto sr = gammaToLinear(c1[2]);
	auto sg = gammaToLinear(c1[1]);
	auto sb = gammaToLinear(c1[0]);
...
void CIELABConvertor::LAB2RGB(Vec4b& pixel, const Lab& lab){
	...
	pixel[0] = clamp((int)rint(b * UCHAR_MAX), 0, UCHAR_MAX);
	pixel[1] = clamp((int)rint(g * UCHAR_MAX), 0, UCHAR_MAX);
	pixel[2] = clamp((int)rint(r * UCHAR_MAX), 0, UCHAR_MAX);
	pixel[3] = clamp((int)lab.alpha, 0, UCHAR_MAX);
}
...
double CIELABConvertor::L_prime_div_k_L_S_L(const Lab& lab1, const Lab& lab2)
{
	...
	auto S_L = 1 + ((0.015 * sqr(barLPrime - 50.0)) / sqrt(20 + sqr(barLPrime - 50.0)));
...
double CIELABConvertor::C_prime_div_k_L_S_L(const Lab& lab1, const Lab& lab2, double& a1Prime, double& a2Prime, double& CPrime1, double& CPrime2)
{
	...
	auto C1 = sqrt((lab1.A * lab1.A) + (lab1.B * lab1.B));
	auto C2 = sqrt((lab2.A * lab2.A) + (lab2.B * lab2.B));
	auto barC = (C1 + C2) / 2.0;
	auto G = 0.5 * (1 - sqrt(pow(barC, 7) / (pow(barC, 7) + pow25To7)));
...
double CIELABConvertor::H_prime_div_k_L_S_L(const Lab& lab1, const Lab& lab2, const double a1Prime, const double a2Prime, const double CPrime1, const double CPrime2, double& barCPrime, double& barhPrime)
{
	...
	auto deltaHPrime = 2.0 * sqrt(CPrimeProduct) * sin(deltahPrime / 2.0);
...
{
	auto pixel2Y = [](const Vec4b& pixel) -> double {
		auto sr = gammaToLinear(pixel[2]);
		auto sg = gammaToLinear(pixel[1]);
		auto sb = gammaToLinear(pixel[0]);
